...
if(jsonlab.jsonopt('Compact',0,opt)==1)
...
jsonp=jsonlab.jsonopt('JSONP','',opt);
if(~isempty(jsonp))
    json=sprintf('%s(%s);%s',jsonp,json,nl);
end

% save to a file if FileName is set, suggested by Patrick Rapin
if(~isempty(jsonlab.jsonopt('FileName','',opt)))
    if(jsonlab.jsonopt('SaveBinary',0,opt)==1)
...
len=numel(item);
ws=jsonlab.jsonopt('whitespaces_',struct('tab',sprintf('\t'),'newline',sprintf('\n'),'sep',sprintf(',\n')),varargin{:});
...
ws=jsonlab.jsonopt('whitespaces_',ws,varargin{:});
padding0=repmat(ws.tab,1,level);
padding2=repmat(ws.tab,1,level+1);
...
if(length(size(item))>2 || issparse(item) || ~isreal(item) || ...
   isempty(item) ||jsonlab.jsonopt('ArrayToStruct',0,varargin{:}))
    ...
    if(numel(item)==1 && jsonlab.jsonopt('NoRowBracket',1,varargin{:})==1 && level>0)
