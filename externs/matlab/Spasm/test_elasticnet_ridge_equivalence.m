...
beta_en = spasm.larsen(X, y, 1e-9, 0, [], false, false);
beta_ridge = (X'*X + 1e-9*eye(p))\X'*y;
assert(norm(beta_en - beta_ridge) < 1e-12);

beta_en = spasm.larsen(X, y, 1e-2, 0, [], false, false);
beta_ridge = (X'*X + 1e-2*eye(p))\X'*y;
assert(norm(beta_en - beta_ridge) < 1e-12);

beta_en = spasm.larsen(X, y, 1e2, 0, [], false, false);
beta_ridge = (X'*X + 1e2*eye(p))\X'*y;
assert(norm(beta_en - beta_ridge) < 1e-12);

beta_en = spasm.larsen(X, y, 1e9, 0, [], false, false);
