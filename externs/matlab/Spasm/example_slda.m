...
Q = 3; % request two discriminative directions
tol = 1e-6;

% normalize training and test data
[X, mu, d] = spasm.normalize(X);
X_test = (X_test-ones(n,1)*mu)./sqrt(ones(n,1)*d);

% run SLDA
[beta, theta] = spasm.slda(X, Y, delta, stop, Q, maxiter, tol, true);
%[beta, theta] = spasm.slda(X, Y, 'delta', delta, 'stop', stop, 'ncomp', Q, 'maxIter', maxiter, 'tol', tol, 'verbose', true );
