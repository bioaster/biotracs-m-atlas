function [SL, SD, L, D, paths] = spca(X, Gram, K, delta, stop, maxSteps, convergenceCriterion, verbose)
% SPCA  The sequential variant of the SPCA algorithm of Zou et. al [1] for
% computing sparse principal components. 
%
%    [SL SV L D PATHS] = SPCA(X, GRAM, K, DELTA, STOP) computes sparse
...
%   Link with SVD
%   [U, S, V'] = svd()
%   [SL, SD, L, D, paths] = spca()
%   D = S, L = V'
%   SD = sparsed S^2 (WRONG: SD = sparsed S^2/n where n = number of instances, D.A. Ouattara, BIOASTER), SL = sparsed V'
%
  ...
  while ~converged && step < maxSteps
    ...
    if delta == inf
      ...
      if stop(k) < 0 && -stop(k) < p
        ...
        SL(:,k) = ( sign(AXX).*max(0, abs(AXX) - sortedAXX(-floor(stop(k)) + 1)) )';
      else
        SL(:,k) = ( sign(AXX).*max(0, abs(AXX) - stop(k)) )';
      end
    else
      % Find beta by elastic net regression
      SL(:,k) = spasm.larsen(X, X*A(:,k), delta, stop(k), Gram, false, false);
    end
...
%     end
    ...
    B_norm = sqrt(SL(:,k)'*SL(:,k));
    if B_norm <= eps(B_norm)
      B_norm = 1;
    end
    SL(:,k) = SL(:,k)/B_norm;
    
    % converged?
    criterion = sum((Bk_old - SL(:,k)).^2);
    ...
    %BugFixed
    %D.A. Ouattara, BIOASTER
    %Also set converged = true if the residual variance is zero
    if sum(S) == 0
        converged = true;
    end
