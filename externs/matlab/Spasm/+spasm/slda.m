function [B, theta, Bpath] = slda(X, Y, delta, stop, Q, maxSteps, convergenceCriterion, verbose)
...
elseif Q > K - 1
    Q = K - 1; 
    fprintf('At most K-1 components allowed. Forcing Q = K - 1.\n');
    %warning('SpaSM:slda', 'At most K-1 components allowed. Forcing Q = K -
    %1.');
end
if nargin < 4
    stop = ceil(p/2);
...
dpi = sum(Y.^2)/n;          % diagonal "matrix" of class priors (Bug fixed: Dpi = (Y'*Y)/n in the paper)
...
for q = 1:Q
    ...
    if verbose
        fprintf('Estimating direction %g', q);
    end
    ...
    while ~converged && step < maxSteps
        ...
        B(:,q) = spasm.larsen(X, Y*theta(:,q), delta, stop(q), [], false, false);
        ...
        if verbose && ~mod(step, 50)
            fprintf('\t iteration: %g , convergence criterion: %g\n', step, criterion );
        end
        converged = criterion < convergenceCriterion;
        if step == maxSteps
            %warning('SpaSM:slda', 'Forced exit. Maximum number of steps reached.');
            fprintf('Warning: Forced exit. Maximum number of steps reached.\n');
        end
    end
  
    if verbose
        fprintf('\t iteration: %g , convergence criterion: %g', step, criterion );
    end
    
    %compute the final solution and store path
    Btmp = spasm.larsen(X, Y*theta(:,q), delta, stop(q), [], true, false); %store paths
    yhatq = X*Btmp;
    t = Ydpi'*yhatq;
    s = t - theta(:,1:q-1)*(theta(:,1:q-1)'*(diag(dpi)*t));
    theta(:,q) = s/sqrt(sum(dpi'.*s.^2));
    ...
    nbVarSelectedAtEachStep = sum(Btmp ~= 0);
    indexesOfStepsToSelect = zeros(1,-stop(q));
    
    for nbVarIdx = 1:-stop(q)
        indexesOfStepsToSelect(nbVarIdx) = find(nbVarSelectedAtEachStep == nbVarIdx, 1, 'last');
    end
    
    %size(B)
    %size(Btmp)

    Bpath{q} = Btmp(:,indexesOfStepsToSelect);
    B(:,q) = Btmp(:,indexesOfStepsToSelect(end));
end
