function [b, steps] = larsen(X, y, delta, stop, Gram, storepath, verbose) %#codegen
...
while length(A) < maxVariables && ~stopCond && step < maxSteps
    ...
    if ~lassoCond
        % add variable
        if ~useGram
            R = spasm.cholinsert(R, X(:,cidx), X(:,A), delta);
        end
        if verbose
    ...
    if isempty(I)
        ...
        if isempty(temp)
            ...
            %fprintf('SpaSM:larsen: Could not find a positive direction towards the next event.\n');
            break;
        end
        gamma = temp(1);
    end
...
%remvoe inactive selections
idx = [];
q = size(b,2);
nbVars = sum( logical(b) );
for i=q:-1:1
    if nbVars(i) == 0
        idx = [idx, i];
    elseif i < q
...
if step == maxSteps
    ...
    fprintf('SpaSM:larsen: Forced exit. Maximum number of steps reached.\n');
end
