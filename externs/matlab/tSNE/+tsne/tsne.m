...
% University of California, San Diego
    ...
    if ~exist('no_dims', 'var') || isempty(no_dims)
     ...
     if ~exist('initial_dims', 'var') || isempty(initial_dims)
        ...
        %BugFix: The maximal dimensional representation cannot be greater than min(N,D)
        %initial_dims = min([50, size(X, 2)]);
        initial_dims = min([50, size(X, 2), size(X, 1)]);
    end
    ...
    P = tsne.d2p(D, perplexity, 1e-5);                                           % compute affinities using fixed perplexity
    clear D
    
    % Run t-SNE
    if initial_solution
        ydata = tsne.tsne_p(P, labels, ydata);
    else
        ydata = tsne.tsne_p(P, labels, no_dims);
    end
